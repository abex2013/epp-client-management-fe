...
export class ViewClientsComponent implements OnInit {
  ...
  PageIndexChange(index: any): void {
    if (this.isFilter) {
      ...
      this.loading = true;
      if (
        this.searchProject.value?.length > 1 &&
        ...
        this._clientservice
          ...
          .subscribe((response: PaginatedResult<Client[]>) => {
            ...
            this.totalRecordBackup = response.pagination.totalRecord;
        ...
        this._clientservice
          ...
          .subscribe((response: PaginatedResult<Client[]>) => {
            ...
            this.loading = false;
            if (
              this.searchAddressList.length > 0 ||
              this.searchstatusList.length > 0 ||
              this.searchsalesPersonList.length > 0
            ) {
              this.search(
  ...
  getLocations() {
    this.operatingAddressService
      .getData()
      .subscribe((res: AllDataResponse<OperatingAddress[]>) => {
        this.locations = res.data;
      });
}
fetchAllData(){
  this.fetchclientsService.getData().pipe(delay(2000)).subscribe((res:AllDataResponse<Client[]>) => {
    ...
    if (
      ...
      const filterFunc = (item: Client) =>
        ...
        (this.searchsalesPersonList.length
          ...
          : true);
      const data = this.allClients.filter((item) => filterFunc(item));
